var Vue = (function(exports) {
  ...
  const transformVHtml = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(53 /* X_V_HTML_NO_EXPRESSION */, loc)
      )
    }
    if (node.children.length) {
      context.onError(
        createDOMCompilerError(54 /* X_V_HTML_WITH_CHILDREN */, loc)
      )
  ...
  const transformVText = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(55 /* X_V_TEXT_NO_EXPRESSION */, loc)
      )
    }
    if (node.children.length) {
      context.onError(
        createDOMCompilerError(56 /* X_V_TEXT_WITH_CHILDREN */, loc)
      )
      node.children.length = 0
    }
    return {
  ...
  const transformModel$1 = (dir, node, context) => {
    ...
    if (tagType === 0 /* ELEMENT */) {
      if (dir.arg) {
        context.onError(
          createDOMCompilerError(58 /* X_V_MODEL_ARG_ON_ELEMENT */, dir.arg.loc)
        )
      }
      if (tag === 'input' || tag === 'textarea' || tag === 'select') {
        ...
        context.onError(
          createDOMCompilerError(57 /* X_V_MODEL_ON_INVALID_ELEMENT */, dir.loc)
        )
      }
    }
    return baseResult
  ...
  const transformShow = (dir, node, context) => {
    ...
    if (!exp) {
      context.onError(
        createDOMCompilerError(60 /* X_V_SHOW_NO_EXPRESSION */, loc)
      )
    }
    return {
      props: [],
  ...
  */
  ...
  function createRenderer(options) {
    ...
    function patchUnkeyedChildren(
      ...
      for (i = 0; i < commonLength; i++) {
        const nextChild = optimized ? c2[i] : (c2[i] = normalizeVNode(c2[i]))
        patch(
          c1[i],
          nextChild,
          container,
    ...
    function patchKeyedChildren(
      ...
      while (i <= e1 && i <= e2) {
        const n1 = c1[i]
        const n2 = optimized ? c2[i] : (c2[i] = normalizeVNode(c2[i]))
      ...
      while (i <= e1 && i <= e2) {
        const n1 = c1[e1]
        const n2 = optimized ? c2[i] : (c2[e2] = normalizeVNode(c2[e2]))
        if (isSameVNodeType(n1, n2)) {
          patch(
            n1,
            n2,
      ...
      if (i > e1) {
        if (i <= e2) {
          ...
          while (i <= e2) {
            patch(
              null,
              optimized ? c2[i] : (c2[i] = normalizeVNode(c2[i])),
              container,
              anchor,
              parentComponent,
              parentSuspense,
  ...
  function resolveAsset(
    type,
    name,
    instance = currentRenderingInstance || currentInstance
  ) {
    if (instance) {
